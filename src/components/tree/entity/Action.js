...
export function createAction(tree) {
    ...
    const appendChild = function (node) {
        if (TreeNode.isType(node)) {
            tree.appendRawNode(this, node.data.raw); // this为当前的TreeNode
            return [node];
        }
    ...
    const removeChild = function (index) {
        tree.removeChildRawNode(this, index); // this为当前的TreeNode
    };
    const insertChild = function (index, node) {
        if (TreeNode.isType(node)) {
            tree.insertRawNode(this, index, node.data.raw); // this为当前的TreeNode
            return [index, node];
        }
